...
开放认领候选人必须同时满足：项目未删除、未归档；存在当前可认领任务或仍有空余区间的翻译任务；用户拥有与任务角色匹配的已审批技能标签。若项目或模板为该角色配置了 `requiredTagIds`，以这些标签为准；否则按标签的 `role_type` 匹配，不依赖标签显示名称。
...
| 正式普通成员 | 允许 | 允许 | 可读；写入版本按 Wiki 审核流程处理 | 仅限已指派、已认领或角色允许的任务 | 无项目管理或任务审核权 |
...
| 整项认领 | 具有匹配已审批标签的成员或项目管理者 | 任务必须为 `claimable`、依赖满足、非翻译任务；并发冲突返回 `409` |
| 翻译分段认领 | 具有翻译标签的成员或项目管理者 | 区间合法、不重叠、未超项目和用户限制；可排队激活 |
